...
public class RedisConfig {
	...
	@Bean
	public RedisCacheUtils cacheUtils(@Qualifier("cabbageRedisPool") JedisPool cabbageRedisPool) {
		return new RedisCacheUtils(namespace, cabbageRedisPool);
	}

	@Bean
	public JedisPool jedisPool() {
		JedisPool pool = new JedisPool(new JedisPoolConfig(), host, port, timeout, password, database);
		return pool;
	...
	public RedisMessageHandler redisMessageHandler() {
		...
		JedisPool pool = new JedisPool(poolConfig, host, port, timeout, password, messageDatabase);
		RedisMessageHandler handler = new RedisMessageHandler(pool);
		return handler;
	}

	@Bean
	public CabinetRemoteHandle cabinetRemoteHandle(
			@Qualifier("redisMessageHandler") RedisMessageHandler redisMessageHandler) {
