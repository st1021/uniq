...
(function(root, factory) {
	...
	var MobileSelectArea = function() {
		...
		this.text = ['', '', ''];
		this.level = 3;
		this.mtop = 40 * $("html").attr("data-dpr");
		this.separator = ' ';
	};
	MobileSelectArea.prototype = {
		init: function(settings) {
			...
			this.text = this.settings.text || (function(val, data) {
                var text = [];
                ...
                $.each(data, function(i) {
                    if (data[i].id == val[0]) {
                        ...
                        if (data[i]["child"]) {
                            $.each(data[i]["child"], function(j) {
                                if (data[i]["child"][j].id == val[1]) {
                                    text.push(data[i]["child"][j].name);
                                    if (data[i]["child"][j]["child"]) {
                                        $.each(data[i]["child"][j]["child"], function(n) {
                                            if (data[i]["child"][j]["child"][n].id == val[2]) {
                                                text.push(data[i]["child"][j]["child"][n].name);
                                            }
                                        });
                                    }
                                }
		...
		bindEvent: function() {
			...
			this.trigger[_this.settings.eventName](function(e) {
				...
				_this.scroller.children().bind('touchend', function(e) {
					...
					var mod = top / _this.scroller.find("p").height();
					...
					dl.css('top', mode * _this.scroller.find("p").height());
                    //console.log(mode * _this.scroller.find("p").height());
					return false;
				});
				return false;
			});
