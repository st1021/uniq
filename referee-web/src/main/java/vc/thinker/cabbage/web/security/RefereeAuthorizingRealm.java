 ...
 * @version 2013-5-29
 */
public class RefereeAuthorizingRealm extends SessionAuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		RefereeBO referee = refereeService.findById(user.getId());
		if(referee == null ){
			throw new UnknownAccountException("The username or password does not exist");
		}
		
		byte[] salt = Encodes.decodeHex(account.getPassword().substring(0,16));
		return new SimpleAuthenticationInfo(new RefereePrincipal(user,referee,account.getLoginName(),token.getHost()), 
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		RefereePrincipal principal = (RefereePrincipal) getAvailablePrincipal(principals);
