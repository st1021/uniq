...
b.val(g+" DESC"!=g+" "+j?"":g+" "+j)}else{b.val(g+" ASC")}a.callBack()})}$(document).ready(function(){$("select").select2();$(".fancybox").fancybox();
    ...
    ;(function() {
        $("select[data-select-url]").each(function() {
            var $this = $(this);
            $.ajax({
                url: $this.data("select-url"),
                success: function(data) {
                    updateSelect(data, $this);
                }
            });
        });
        function updateSelect (data, $select) {
            var html = "<option></option>", checked = "", pdate = {}, pHtml = "";
            if (data.length > 0) {
                $.each(data, function(i) {
                    pdate[data[i].id] = data[i].name;
                    if ($select.data("select-value") == data[i].id) {checked = "selected";} else { checked = ""};
                    if (pdate[data[i].pId]) {pHtml = " (" + pdate[data[i].pId] + ")";  } else { pHtml = ""};
                    html += "<option id='" + data[i].id + "' value='" + data[i].id + "' class='area-select-level" + data[i].id.split("-").length + "' "+ checked +">" + data[i].name + pHtml + "</option>";
                });
            }
            $select.html(html).change();
        }
    }());
