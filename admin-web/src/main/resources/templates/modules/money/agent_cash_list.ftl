...
<head>
	<script type="text/javascript">
	    ...
	    {
	    	// 正常打开
			top.$.jBox.open("iframe:${ctx}/sys/userMoneyCash/checkForm?id="+id, "Agent Withdrawal", 500, 250, {
					            ...
					            if (v === 'ok') {
					                 $.ajax({
					                 	...
					                 	success:function(data){
					                 		...
					                 		{
					                 			top.$.jBox.close();
					                 			top.$.jBox.tip("Congratulations, the success of the operation");
	            ...
	            if (v === 'ok') {
	            	  ...
	            	  $.ajax({
			                 	...
			                 	success:function(data){
			                 		...
			                 		window.location.href="${ctx}/sys/userMoneyCash/agentCash";
			                 	}
			          	 });
	            }
	            return true;
	...
	<div class="page-container-custom">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <span><@spring.message code="financial.withdrawal.agent" /></span>
                </li>
            </ul>
        </div>
        ...
        <@form.form id="searchForm" action="${ctx}/sys/userMoney/agentCash" method="post" class="breadcrumb form-search">
        	<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo!}"/>
			<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize!}"/>
			<@spring.message code="userInfo.agent.name" />:<input type="text" name="agentName" value="${vo.agentName!}" maxlength="50" class="form-control input-small input-inline"/>
			&nbsp;&nbsp;
			<@spring.message code="sys.mobile"></@spring.message>:<input type="text" name="contactMobile" value="${vo.contactMobile!}" maxlength="15" class="form-control input-small input-inline"/>
			&nbsp;&nbsp;
			...
			<select id="cashPaymentType" name="cashPaymentType" class="select2 form-control input-small">
				<option value="">All</option>
    	 		<option value="alipay" <#if vo.cashPaymentType ?? && vo.cashPaymentType == "alipay">selected='selected'</#if>>支付宝</option>
    	 		<option value="chinabank" <#if vo.cashPaymentType ?? && vo.cashPaymentType == "chinabank">selected='selected'</#if>>网银在线</option>
    	 	</select>
    	 	-->
    	 	<br/></br/>
			<@spring.message code="financial.deposit.applyTime" />：
          ...
          <div class="portlet-body">
            ...
            <#if page.list?size &gt; 0 >
                <div class="table-scrollable">
					...
					<tr>
						<th width="8%"><@spring.message code="userInfo.agent.name" /></th>
			            <th width="5%"><@spring.message code="userInfo.merchant.contactmobile" /></th>
			            ...
			            <th width="5%"><@spring.message code="form.status" />Status</th>
			            <th width="12%"><@spring.message code="ffinancial.deposit.applyTime" /></th>
